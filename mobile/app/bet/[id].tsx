...
export default function BetDetailScreen() {
  ...
  const handleJuryApprove = useCallback(() => {
    if (!currentBet || !id) return;
    // Need to pick the winner when approving
  ...
  // All people eligible to be declared the winner
  ...
  return (
    <>
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        {/* Evidence */}
        {currentBet.evidence && currentBet.evidence.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.sectionLabel}>{t('evidence')}</Text>
            {currentBet.evidence.map((ev) => (
